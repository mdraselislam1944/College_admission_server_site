...
async function run() {
  try {
    ...
    app.get("/editInformation/:id", async (req, res) => {
      const id = req.params.id;
      const result = await studentInformation.findOne({ _id: new ObjectId(id) });
      res.send(result);
    })
    app.patch('/updateStudent/:id', async (req, res) => {
      ...
      const result = await studentInformation.updateOne(filter, updateProduct, option);
      res.send(result);
    })

    const Feedback = client.db("MRGOVTCOLLEGEDB").collection("feedback");

    app.post("/information",async(req,res)=>{
      const body=req.body;
      const result=await Feedback.insertOne(body);
      res.send(result);
    })
